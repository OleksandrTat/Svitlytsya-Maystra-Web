      ...
      <AdminActionForm action={upsertProjectAction} submitLabel="Зберегти проєкт">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <select
            ...
            className="rounded-xl border border-[var(--color-border)] px-3 py-2 text-sm"
          >
            {Object.entries(PROJECT_PRIVACY_LEVEL_LABELS).map(([value, label]) => (
              <option key={value} value={value}>
                {label}
              </option>
            ))}
          ...
          <input
            name="blurred_images"
            placeholder="Blurred image URL через кому"
            className="rounded-xl border border-[var(--color-border)] px-3 py-2 text-sm md:col-span-2"
          />
          <input
