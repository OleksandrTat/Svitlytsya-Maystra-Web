...
const DRAFT_STORAGE_KEY = "admin-blog-post-draft-v1";
...
function readDraft(mode: "create" | "edit") {
  ...
  try {
    ...
    return JSON.parse(raw) as Partial<BlogDraft>;
  } catch {
    return null;
  }
}
...
export function BlogPostForm({ mode, initialData }: BlogPostFormProps) {
  ...
  const [category, setCategory] = useState(initialData?.category ?? initialDraft?.category ?? "materials");
  ...
  const [savedSnapshot, setSavedSnapshot] = useState(() =>
    JSON.stringify({
      ...
      category: initialData?.category ?? initialDraft?.category ?? "materials",
  ...
  const formTitle = useMemo(() => (mode === "create" ? "Нова стаття" : "Редагування статті"), [mode]);
  ...
  }, [mode, title, slug, excerpt, content, coverImage, category, tags, seoTitle, seoDescription, isPublished]);
  ...
  const currentSnapshot = useMemo(
    ...
    [title, slug, excerpt, content, coverImage, category, tags, seoTitle, seoDescription, isPublished],
  );
    ...
    <form ref={formRef} onSubmit={onSubmit} className="space-y-6">
      <div className="sticky top-3 z-20 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--color-border)] bg-white/95 p-4 shadow-sm backdrop-blur">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Link href="/admin/blog" className="rounded-lg border border-[var(--color-border)] px-3 py-2 text-sm">
            До списку
          </Link>
